...
<%--				<p>酒店名称</p>--%>
<%--				<p>酒店价格:</p>--%>
<%--				<a class="btn btn-sm btn-success" href="${pageContext.request.contextPath}/scenic/scenicId">查看详情</a>--%>
<%--			</div>--%>
		</div>
	</div>
	...
	<script>
		...
		function build_ScenicList(data) {//将json中的数据通过jquery遍历添加到table上显示出来
			$("#scenicRow").empty();//清空上一次查询到的数据，防止重叠

			var scenics=data.extend.pageInfo.list;
			$.each(scenics,function (index,item) {
				var scenicImg=$("<img style='border-radius: 16px' width=\"240px\" height=\"200px\"></img>").attr("src","${pageContext.request.contextPath}/img/Scenic/"+item.scenicImage+"");
				var scenicName=$("<p style='color: white'></p>").append("景点名称："+item.scenicName);
				var scenicPrice=$("<p style='color: white'></p>").append("门票价格："+item.scenicPrice);
				var scenicText=$("<p style='color: white'></p>").append("开放时间：9:00-16:10");
				var scenicText2=$("<p style='color: white'></p>").append("温馨提示：景区门票当日一次进入有效！");
				var scenicView=$("<a class='btn btn-sm btn-default'></a>").append("查看详情").attr("href","${pageContext.request.contextPath}/openScenicDetailPage"+"?scenicId="+item.scenicId+"");
				$("<div class=\"col-md-2 col-md-offset-1\" style=\"text-align:right;margin-top: 40px\"></div>").append(scenicImg).appendTo("#scenicRow");
				$("<div class=\"col-md-2 \" style=\"border-left: 1px #FFFFFF dashed; border-radius: 16px; background-color:#3d4444;width: 360px;height: 200px;margin-top: 40px\"></div>").append(scenicName).append(scenicPrice).append(scenicText).append(scenicText2).append(scenicView).appendTo("#scenicRow");
			});
		}
		...
		function to_page(pn) {
			$.ajax({
				url:"${pageContext.request.contextPath}/loadScenicList",
				data:"pn="+pn,
				type:"get",
