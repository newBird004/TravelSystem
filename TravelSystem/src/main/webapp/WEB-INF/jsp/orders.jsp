...
<div class="row">
    <div class="col-md-12" style="font-size: 20px;background-color:black; border-radius:20px;margin-bottom: 5px">
        <span style="color: #FFFFFF">&nbsp;&nbsp;订单管理</span>&nbsp;&nbsp;
    </div>
</div>
<!-- 按钮 -->
<div class="row">
    ...
    <div class="col-md-1 col-md-offset-1">
        <button type="button" class="btn btn-success" id="selectAllOrder">显示所有订单</button>
    </div>

    <div class="col-md-4 col-md-offset-2">
        <input type="text" placeholder="输入要搜索的订单ID" id="inputOrderIdToSearch">
        <button type="button" class="btn btn-default" id="selectOrderByCondition"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></button>
    </div>

    <div class="col-md-4">
        <input type="text" placeholder="输入要搜索的用户编号" id="inputUserIdToSearch">
        <button type="button" class="btn btn-default" id="selectOrderByUserId"><span class="glyphicon glyphicon-search" aria-hidden="true"></span></button>
    </div>
</div>
<!-- 表格数据 -->
<div class="row">
...
<script type="text/javascript">
    ...
    $("#selectOrderByCondition").click(function () {
        ...
        $.ajax({
            url:"${APP_PATH}/selectOrderById?orderId="+$("#inputOrderIdToSearch").val()+"",
            type:"GET",
    ...
    /*根据用户编号搜索订单*/
    $("#selectOrderByUserId").click(function () {
        if($("#inputUserIdToSearch").val()==""){
            alert("请输入要查询的用户编号！");
            return false;
        }
        $.ajax({
            url:"${APP_PATH}/selectOrderByUserId?userId="+$("#inputUserIdToSearch").val()+"",
            type:"GET",
            success:function(result){//根据拿到的信息显示到模态框中
                build_OrderListByUserId(result);
                $("#pageInfo").empty();
                $("#pageNav").empty();
            }
        });
    ...
    function build_OrderListByUserId(data) {//将json中的数据通过jquery遍历添加到table上显示出来
        $("#orderTable tbody").empty();//清空上一次查询到的数据，防止重叠

        var orders=data.extend.orders;
        $.each(orders,function (index,item) {
            var orderIdTd=$("<td></td>").append(item.orderId);
            var travelUserIdTd=$("<td></td>").append(item.travelUserId_FK_Orders);
            var orderDateTd=$("<td></td>").append(item.orderDate);
    ...
    function build_OrderDetailList(data) {//将json中的数据通过jquery遍历添加到table上显示出来
        ...
        var orderItems=data.extend.orderItems;
            ...
            $.each(orderItems,function (index,item) {
                ...
                var goodsNameTd=$("<td></td>").append(item.goodsName+"("+item.goodsId+")");
